...
async def merge_files(data: MergeRequest):
    try:
        ...
        print(f"[MERGE START] Folder ID: {folder_id}")
        print(f"[MERGE START] File IDs: {file_ids}")
        ...
        for file_id in file_ids:
            ...
            try:
                ...
                print(f"[ERROR] Parsing failed for blob: {blob_path}")
                traceback.print_exc()
                raise HTTPException(status_code=500, detail=f"Error parsing {blob_path}: {err}")

            if df.empty:
                print(f"[WARNING] Skipping empty file: {blob_path}")
                continue

            # Calculate sampling interval if not set
        ...
        print(f"[MERGE COMPLETE] Saved as: {merged_filename}")
